...
AWS_REGION=$(read_var AWS_REGION .env.production)
...
aws ecr get-login-password --region $AWS_REGION | docker login --username AWS --password-stdin $AWS_ECR_REPO_URI
docker build --no-cache	-f $ROOT_DIR/Dockerfile.prod -t $AWS_ECR_REPO_NAME .
docker tag $AWS_ECR_REPO_NAME:latest $AWS_ECR_REPO_URI:latest
